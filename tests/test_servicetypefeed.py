...
class ParseWebApi(unittest.TestCase):
    def setUp(self):
        ...
        self.logger = logger
        self.maxDiff = None
        self.services_webapi = ParseWebApiServiceTypes(service_types)
    ...
    def test_FailedWebApiFeedParse(self):
        with self.assertRaises(ConnectorParseError) as cm:
            fail_services_webapi = ParseWebApiServiceTypes('FAILED_DATA')
...
class ParseGocdb(unittest.TestCase):
    def setUp(self):
        with open('tests/sample-service_types_gocdb.xml', encoding='utf-8') as feed_file:
            service_types = feed_file.read()
        self.services_gocdb = ParseGocdbServiceTypes(service_types)
        self.maxDiff = None
        self.fail_services_gocdb = ParseGocdbServiceTypes('FAILED_DATA')
...
class ParseFlat(unittest.TestCase):
    def setUp(self):
        ...
        self.services_flat = ParseFlatServiceTypes(service_types, is_csv=True)
        self.maxDiff = None
    ...
    def test_FailedFeedParse(self, mocked_csv2json):
        ...
        services_flat = ParseFlatServiceTypes(self.service_types, is_csv=True)
        with self.assertRaises(ConnectorParseError) as cm:
